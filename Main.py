...
def get_conv(input_shape=(64, 64, 3), filename=None):
    ...
    model.add(Conv2D(32, (3, 3), activation='leaky_relu', name='conv1', input_shape=input_shape, padding="same"))
    model.add(Conv2D(64, (3, 3), activation='leaky_relu', name='conv2', padding="same"))
    model.add(MaxPooling2D(pool_size=(3, 3)))
    model.add(Dropout(0.2))
    model.add(Conv2D(128, (8, 8), activation="leaky_relu", name="dense1"))
    model.add(Dropout(0.2))
...
annotated, heatmap = locate('4.jpg', filepath="Data/Raw/Test/")
Image.fromarray(annotated).show()
